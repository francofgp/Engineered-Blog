...
- **Wordmark motion (living wordmark):** on fine-pointer devices the name reacts to the cursor — each glyph thickens along Archivo's variable weight axis (rest 700 → peak 800, "ink pooling under the hand") and the ocre squares scale a touch (`--wm-pulse`). Interaction-driven, state-change motion only — no colour shift, no ambient/scroll animation; rest is pixel-identical to the static 700 mark. Gated off (and re-evaluated at runtime) under `prefers-reduced-motion` and on touch/coarse pointers; degrades to plain static text with no JS. Two-way guard: don't strip it as "motion for its own sake" (it's gated + interaction-only), and don't escalate it past 800 or into colour/scroll. See `docs/THEME.md` History → 2026-05-30 "Living wordmark".

### Footer

- **Separation:** set off from the page body by a single **container-width** `--eb-border` hairline at the top of the footer's `.container` (same token *and* width as the colophon divider below it), with `section-sm` (80px) top padding. The border lives on the inner `.container`, not the `<footer>` element, so it isn't a full-bleed line inconsistent with the contained colophon. Hairline + rhythm only — no background, no colour shift. Markup: `<footer class="site-footer">`; rule in `templates/_main.scss`. (Earlier this was an ad-hoc `mt-2` margin with no divider.) See `docs/THEME.md` History → 2026-05-30 "Header + footer polish".
- **Structure:** a four-column navigation block (Contact Me / Social Contacts / Categories / Quick Links) — social is plain text links, not icon pills (see §Tag Pills) — closed by the centered colophon bar (`.footer-colophon`, muted text + ocre year).
